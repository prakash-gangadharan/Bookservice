...
import com.webflux.funcendpoint.model.ProductEvent;
...
public class ProductHandler {
    ...
    public Mono<ServerResponse> updateProduct(ServerRequest request) {
        ...
        return 
        		productMono.zipWith(existingProductMono,(product, existingProduct) ->
                        new Product(existingProduct.getId(), product.getName(), product.getPrice()))
              .log()
              .flatMap(product1 ->
                        ServerResponse.ok()
                                .contentType(APPLICATION_JSON)
                                .body(repository.save(product1), Product.class)
                                .log()
              );
              //.switchIfEmpty(notFound);
    }
